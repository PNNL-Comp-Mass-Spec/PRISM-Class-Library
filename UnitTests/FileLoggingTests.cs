...
namespace PRISMTest
{
    ...
    internal class FileLoggingTests
    {
        ...
        public void TestArchiveOldLogFiles(string logDirectory, int yearsToSimulate)
        {
            ...
            Assert.That(logFilesCreated1, Is.Not.Empty, "CreateLogFiles returned an empty list for logFilesCreated1");
            ...
            Assert.That(logFilesCreated2, Is.Not.Empty, "CreateLogFiles returned an empty list for logFilesCreated2");
            ...
            foreach (var oldLogFilesDir in logFileDirs)
            {
                ...
                Assert.That(fileCountInZip, Is.GreaterThanOrEqualTo(FILES_PER_YEAR), $"Zip file {zipFileToCheck.FullName} has fewer than {FILES_PER_YEAR} files");
        ...
        private void EvaluateFormattedMessageTimestamp(string formattedMessage, string expectedFormatString, bool useLocalTime)
        {
            ...
            Assert.That(Math.Abs(timeDiffSeconds), Is.LessThanOrEqualTo(2),
